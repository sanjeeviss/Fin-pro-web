...
const EditCustomer = ({ handleRefresh, data }) => {
    ...
    if (open && data) {
      const formattedData = {
        ...
        starting_date: data.starting_date
          ? dayjs(data.starting_date)
          : undefined,
        ending_date: data.ending_date ? dayjs(data.ending_date) : undefined,
        collection_agent: data.collection_agent.id || null,
  ...
  const onFinish = (values) => {
    ...
    UpdateCustomer(data.c_id, newData)
      .then(() => {
        handleRefresh();
        setLoading(false);
        onClose();
      ...
      .catch(() => {
        setLoading(false);
        message.error("Facing error while updating Customer");
      });
  };
